...
OFFICIAL_AIME_STYLE_SEED_PROMPT = {
    "system_prompt": "You are a helpful assistant. You are given a question and you need to answer it. The answer should be given at the end of your response in exactly the format '### <final answer>'"
}
SIMPLE_OK_PROMPT = "Return exactly: OK"
LEVEL_1_PROMPT = "What is 2+3? Put your final answer in the format ### <answer>"
LEVEL_2_PROMPT = (
    "A sequence starts with 2 and increases by 3 each step. "
    "What is the 5th term? Put your final answer in the format ### <answer>"
)
DEFAULT_MODEL = "mimo-v2.5-pro"
DEFAULT_TIMEOUT_SECONDS = 120.0
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "outputs" / "mimo_prompt_complexity_decomposition"
PATH_TYPE = "stage2a_mimo_prompt_complexity_decomposition"
...
def load_first_val_sample_via_init_dataset() -> tuple[dict[str, Any], str]:
    ...
    if not valset:
        raise RuntimeError("官方 `init_dataset()` 返回的 valset 为空，无法执行 prompt complexity diagnosis。")
...
def build_decomposition_levels(aime_question: str) -> list[dict[str, Any]]:
    return [
        {
            "level_id": 0,
            "prompt_type": "simple_ok_prompt",
            "messages_shape": "user_only",
            "messages": [{"role": "user", "content": SIMPLE_OK_PROMPT}],
        },
        {
            "level_id": 1,
            "prompt_type": "very_short_math_prompt",
            "messages_shape": "user_only",
            "messages": [{"role": "user", "content": LEVEL_1_PROMPT}],
        },
        {
            "level_id": 2,
            "prompt_type": "short_aime_like_prompt",
            "messages_shape": "user_only",
            "messages": [{"role": "user", "content": LEVEL_2_PROMPT}],
        },
        {
            "level_id": 3,
            "prompt_type": "real_aime_question_only",
            "messages_shape": "user_only",
            "messages": [{"role": "user", "content": aime_question}],
        },
        {
            "level_id": 4,
            "prompt_type": "readme_seed_prompt_plus_real_aime_question",
            "messages_shape": "system_plus_user",
            "messages": [
                {"role": "system", "content": README_QUICKSTART_SEED_PROMPT["system_prompt"]},
                {"role": "user", "content": aime_question},
            ],
        },
        {
            "level_id": 5,
            "prompt_type": "official_aime_style_seed_prompt_plus_real_aime_question",
            "messages_shape": "system_plus_user",
            "messages": [
                {"role": "system", "content": OFFICIAL_AIME_STYLE_SEED_PROMPT["system_prompt"]},
                {"role": "user", "content": aime_question},
            ],
        },
    ]
...
def run_level_diagnosis(
    *,
    ...
    blocked = not (direct_sdk_result["ok"] and litellm_result["ok"])
    return {
...
def build_decomposition_payload(
    *,
    ...
    first_blocked_level = next((item["level_id"] for item in level_results if item["blocked"]), None)
    return {
        "provider": "mimo",
        ...
        "interpretation": {
            ...
            "blocker_detected": first_blocked_level is not None,
...
def run_prompt_complexity_decomposition(
    *,
    api_key: str,
    api_base: str,
    model: str,
    ...
    if not api_key.strip():
        raise ValueError("缺少 MIMO_API_KEY，无法执行 prompt complexity decomposition 诊断。")
    if not model.strip():
        raise ValueError("缺少模型名，无法执行 prompt complexity decomposition 诊断。")
    ...
    levels = build_decomposition_levels(str(sample["input"]))
    level_results = [
        run_level_diagnosis(
            api_key=api_key,
            api_base=api_base,
    ...
    write_json(run_dir / "prompt_complexity_decomposition.json", payload)
    write_text(
        run_dir / "notes.md",
        "\n".join(
            [
                "# Stage 2A MiMo prompt complexity decomposition",
                "",
                ...
                "- 本脚本依次记录 Level 0 到 Level 5 的默认 generation 结果。",
                "",
            ]
        ),
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="诊断 MiMo strict default path 在 Level 0 到 Level 5 prompt complexity 下的默认 completion 行为；不进入 gepa.optimize。"
    )
    ...
    parser.add_argument(
        ...
        help="输出目录，默认 outputs/mimo_prompt_complexity_decomposition。",
    )
    args = parser.parse_args()

    load_dotenv()
    api_key = _env("MIMO_API_KEY")
    run_dir = run_prompt_complexity_decomposition(
        api_key=api_key,
    ...
    print(
        json.dumps(
            {
                "run_dir": str(run_dir),
                "result_file": str(run_dir / "prompt_complexity_decomposition.json"),
                "path_type": PATH_TYPE,
            },
            ensure_ascii=False,
            indent=2,
